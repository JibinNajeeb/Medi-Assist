...
public class VoiceHelper{
    ...
    private VoiceHelper(Context context,TextToSpeech.OnInitListener initListener){
        mContext = context.getApplicationContext();
        mTextToSpeech = new TextToSpeech(mContext, initListener);
        mTextToSpeech.setPitch(.8f);
        mTextToSpeech.setSpeechRate(1.5f);
        mTextToSpeech.setOnUtteranceProgressListener(mUtteranceProgressListener);
    }
    ...
    public static VoiceHelper getInstance(Context context, TextToSpeech.OnInitListener initListener){
        if(mInstance == null){
            mInstance = new VoiceHelper(context, initListener);
        }

        return mInstance;
    }

    public void setInialized(boolean status) {
        mIsIntialized = true;
    }

    public boolean getInialized() {
        return mIsIntialized;
    }
