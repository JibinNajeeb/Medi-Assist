package com.hack.android.alexa;
...
import com.hack.android.alexa.callbacks.AsyncCallback;
import com.hack.android.alexa.callbacks.AuthorizationCallback;
...
import com.hack.android.alexa.interfaces.GenericSendEvent;
...
import com.hack.android.alexa.interfaces.speechrecognizer.SpeechSendAudio;
import com.hack.android.alexa.interfaces.speechrecognizer.SpeechSendText;
import com.hack.android.alexa.interfaces.speechrecognizer.SpeechSendVoice;
...
import com.hack.android.alexa.service.DownChannelService;
import com.hack.android.alexa.system.AndroidSystemHandler;
import com.hack.android.alexa.utility.Util;
...
import static com.hack.android.alexa.interfaces.response.ResponseParser.getBoundary;
...
public class AlexaManager {
    ...
    private static CustomJsonResponse mCustomResponseGetter;

    public void setAVSResponseGetter(CustomJsonResponse customResponseGetter) {
        mCustomResponseGetter = customResponseGetter;
    }
    ...
    /**
     * Send a synchronize state {@link Event} request to Alexa Servers to retrieve pending {@link com.hack.android.alexa.data.Directive}
    ...
    public void sendAudioRequest(final byte[] data, @Nullable final AsyncCallback<AvsResponse, Exception> callback){
        Log.d("AlexaManager", "sendAudioRequest: ");

        sendAudioRequest(new DataRequestBody() {
            @Override
            public void writeTo(BufferedSink sink) throws IOException {
                Log.d("AlexaManager", "writeTo: sendAudioRequest");
                sink.write(data);
            }
        }, callback);
    }
        ...
        mAuthorizationManager.checkLoggedIn(mContext, new ImplCheckLoggedInCallback() {
            ...
            public void success(Boolean result) {
                if (result) {
                    ...
                    //set our URL
                    final String url = getEventsUrl();
                    Log.d(TAG, "sendAudioRequest: "+url);
                    //get our access token
                    TokenManager.getAccessToken(mAuthorizationManager.getAmazonAuthorizationManager(), mContext, new TokenManager.TokenCallback() {
                        @Override
                        public void onSuccess(final String token) {
                            ...
                            new AsyncTask<Void, Void, AvsResponse>() {
                                ...
                                @Override
                                protected void onPostExecute(AvsResponse avsResponse) {
                                    Log.d(TAG, "onPostExecute: "+avsResponse);
                                    super.onPostExecute(avsResponse);
                                }
                            }.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                        }
     ...
     * See: {@link #sendEvent(String, AsyncCallback)}
     *
     * @param volume volume as reported by the {@link com.hack.android.alexa.interfaces.speaker.AvsAdjustVolumeItem} Directive
     ...
     * See: {@link #sendEvent(String, AsyncCallback)}
     *
     * @param isMute mute state as reported by the {@link com.hack.android.alexa.interfaces.speaker.AvsSetMuteItem} Directive
     * @param callback
     */
    ...
    /**
     * Send a generic event to the AVS server, this is generated using {@link com.hack.android.alexa.data.Event.Builder}
                    ...
                    TokenManager.getAccessToken(mAuthorizationManager.getAmazonAuthorizationManager(), mContext, new TokenManager.TokenCallback() {
                        ...
                        public void onSuccess(final String token) {
                            ...
                            new AsyncTask<Void, Void, AvsResponse>() {
                                @Override
                                protected AvsResponse doInBackground(Void... params) {
                                    Log.i(TAG, "authorizemangaer sendEvent");
                                    Log.i(TAG, event);
    ...
    public String getUrlEndpoint(){
        Log.d(TAG, "getUrlEndpoint: ");
        return urlEndpoint;
    }

    public String getPingUrl(){
        Log.d(TAG, "getPingUrl: ");
    ...
    private static class AsyncEventHandler implements AsyncCallback<Call, Exception>{
        ...
        public void success(Call currentCall) {
            try {
                ...
                Log.d(TAG, "for checkng stopCapture ");
                final AvsResponse items = response.code() == HttpURLConnection.HTTP_NO_CONTENT ? new AvsResponse() :
                        ResponseParser.parseResponse(response.body().byteStream(), getBoundary(response), mCustomResponseGetter);

                Log.d(TAG, "success: Response="+response.body().byteStream());
                Log.i(TAG, "Audio response: " + response.toString());
                Log.i(TAG, "Audio response: " + response.code());
                response.body().close();
