package com.hack.android.alexa.interfaces.response;
...
import com.hack.android.alexa.interfaces.AvsException;
...
import com.hack.android.alexa.interfaces.alerts.AvsDeleteAlertItem;
import com.hack.android.alexa.interfaces.alerts.AvsSetAlertItem;
import com.hack.android.alexa.interfaces.audioplayer.AvsPlayAudioItem;
import com.hack.android.alexa.interfaces.audioplayer.AvsPlayRemoteItem;
import com.hack.android.alexa.interfaces.errors.AvsResponseException;
import com.hack.android.alexa.interfaces.playbackcontrol.AvsMediaNextCommandItem;
import com.hack.android.alexa.interfaces.playbackcontrol.AvsMediaPauseCommandItem;
import com.hack.android.alexa.interfaces.playbackcontrol.AvsMediaPlayCommandItem;
import com.hack.android.alexa.interfaces.playbackcontrol.AvsMediaPreviousCommandItem;
import com.hack.android.alexa.interfaces.playbackcontrol.AvsReplaceAllItem;
import com.hack.android.alexa.interfaces.playbackcontrol.AvsReplaceEnqueuedItem;
import com.hack.android.alexa.interfaces.playbackcontrol.AvsStopItem;
import com.hack.android.alexa.interfaces.speaker.AvsAdjustVolumeItem;
import com.hack.android.alexa.interfaces.speaker.AvsSetMuteItem;
import com.hack.android.alexa.interfaces.speaker.AvsSetVolumeItem;
import com.hack.android.alexa.interfaces.speechrecognizer.AvsExpectSpeechItem;
import com.hack.android.alexa.interfaces.speechrecognizer.AvsStopCaptureItem;
import com.hack.android.alexa.interfaces.speechsynthesizer.AvsSpeakItem;
import com.hack.android.alexa.interfaces.system.AvsSetEndpointItem;
...
public class ResponseParser {
    ...
    public static AvsResponse parseResponse(InputStream stream, String boundary, CustomJsonResponse mCustomResponseGetter) throws IOException, IllegalStateException, AvsException {
        return parseResponse(stream, boundary, false, mCustomResponseGetter);
    }

    public static AvsResponse parseResponse(InputStream stream, String boundary, boolean checkBoundary, CustomJsonResponse mCustomResponseGetter) throws IOException, IllegalStateException, AvsException {
        ...
        Log.i(TAG, "AvsResponsesss="+responseString);
        if (checkBoundary) {
            ...
            while (count < 1 || mpStream.readBoundary()) {
                ...
                if (!isJson(headers)) {
                    ...
                    if(contentId != null) {
                        ...
                        Log.d(TAG, "parseResponse: ="+matcher.toString());
                        if (matcher.find()) {
                            String currentId = "cid:" + matcher.group(1);
                            Log.d(TAG, "parseResponse: ="+currentId);
                    ...
                    Directive aDirective = getDirective(directive);
                    if(aDirective.getHeader().getNamespace().equalsIgnoreCase("TemplateRuntime"))
                        if(mCustomResponseGetter != null)
                            mCustomResponseGetter.jsonText(aDirective);
        ...
        for (Directive directive: directives) {
            Log.i(TAG, "Directive For: " + directive.toString());
    ...
    public static String getBoundary(Response response) throws IOException {
        ...
        Log.d(TAG, "getBoundary: ="+boundary);
        return boundary;
    }
    ...
    public static Directive getDirective(String directive) throws AvsException, IllegalStateException {
        Log.i(TAG, "getDirective="+directive);
        Gson gson = new Gson();
