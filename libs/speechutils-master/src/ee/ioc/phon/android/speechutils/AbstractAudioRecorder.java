...
public abstract class AbstractAudioRecorder implements AudioRecorder {
    ...
    protected AbstractAudioRecorder(int audioSource, int sampleRate) {
        ...
        android.util.Log.d("abstractaudiorecorder", "AudioRecord: "+mOneSec*35);
    }
    ...
    protected int read(SpeechRecord recorder, byte[] buffer) {
        ...
        if (status == 0 && numOfBytes >= 0) {
            ...
            android.util.Log.d("abstractaudiorecorder", "read: ="+mRecording);
            android.util.Log.d("abstractaudiorecorder", "mRecordedLength: ="+mRecordedLength);
            mRecordedLength += len;
        }
        return status;
    }
